    ...
    <Card className="mx-auto max-w-lg border-border bg-card">
      ...
      <CardContent>
        <form onSubmit={onStart} className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-muted-foreground">Incremento por puja (XP)</label>
            <Input
              type="number"
              min={1}
              value={subasta.incremento}
              onChange={(e) => onUpdateSubasta({ incremento: parseInt(e.target.value) || 10 })}
              className="bg-input text-foreground"
            />
          </div>
          <Button type="submit" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-bold text-base py-5">
...
export function AuctionLive({ subasta, timeLeft, cursoData, onBid, onFinish }: AuctionLiveProps) {
  ...
  const ganadorName = subasta.ganadorIdx !== null ? cursoData.alumnos[subasta.ganadorIdx]?.nombre : null
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="mb-2 flex items-center justify-center gap-2">
            <Timer className={`size-5 ${isUrgent ? "text-destructive" : "text-muted-foreground"}`} />
          ...
          {cursoData.alumnos.map((al, i) => {
            ...
            return (
              <Button
                ...
                variant="ghost"
              >
                <span className="text-sm font-bold truncate max-w-full">{al.nombre}</span>
                <Badge
                  className={`text-xs ${
                    isLeader
