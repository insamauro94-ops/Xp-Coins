...
export function StudentClassView({ curso }: StudentClassViewProps) {
  ...
  const totalXP = cursoData.alumnos.reduce((sum, a) => sum + a.xp, 0)
      ...
      <main className="mx-auto max-w-lg px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-border bg-card p-4 text-center">
            <p className="text-xs text-muted-foreground uppercase tracking-wider">Alumnos</p>
            <p className="mt-1 font-mono text-2xl font-bold text-foreground">
              {cursoData.alumnos.length}
            </p>
          </div>
          <div className="rounded-xl border border-border bg-card p-4 text-center">
          ...
          {sorted.length === 0 ? (
            ...
            <div className="space-y-2">
              {sorted.map((al, rank) => (
                <div
                  key={al.originalIdx}
                  className={`flex items-center gap-4 rounded-xl border p-4 transition-all ${getRankBg(rank)}`}
                >
